...
import { Cpu, User, ExternalLink, Zap, Copy, Check } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 space-y-2">
        {tx.nftName ? <Row label="NFT" value={tx.nftName} mono={false} /> : null}
        <Row
          label="Tx Hash"
          value={short(tx.txHash)}
          mono
          action={
            <div className="flex items-center gap-1">
              <button
                ...
                onClick={() => void handleCopy(tx.txHash, 'hash', 'Tx hash')}
        ...
        {tx.gasUsed ? <Row label="Gas Used" value={tx.gasUsed} mono /> : null}
        ...
        {tx.explorerUrl ? (
          <div className="flex flex-wrap gap-2 pt-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-[10px] font-bold uppercase tracking-wider text-blue-400 hover:text-blue-300 border border-blue-500/30 rounded-lg px-2.5 py-1"
            >
              View on Explorer
            </a>
            <button
              type="button"
              onClick={() => void handleCopy(tx.explorerUrl, 'url', 'Explorer URL')}
              className="text-[10px] font-bold uppercase tracking-wider text-[#71717A] hover:text-white border border-[#2A2A32] rounded-lg px-2.5 py-1"
            >
              {copiedField === 'url' ? 'Copied URL' : 'Copy Explorer URL'}
            </button>
          </div>
        ) : null}
      </div>
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <div className="flex items-center gap-1.5 min-w-0">
        <span className={cn('text-xs text-[#A1A1AA] truncate', mono && 'font-mono')}>
          {value}
        </span>
        {action}
      </div>
    ...
    >
      ...
      <div className={cn('flex flex-col gap-1', isAssistant ? 'max-w-[78%]' : 'items-end max-w-[72%]')}>
        ...
        >
          ...
          {message.transaction && <TransactionReceiptCard tx={message.transaction} />}
        </div>
